...
public class FoodItemTester
{
			...
			throws NoSuchFieldException, SecurityException, IllegalArgumentException, IllegalAccessException
	{
		...
		printTestData("constractor aliasing", "d1 == tomato.getProductionDate()", d1 == tomato.getProductionDate(),
				false);
		printTestData("constractor aliasing", "d2 == tomato.getExpiryDate()", d2 == tomato.getExpiryDate(), false);
		printTestData("copy constractor aliasing", "tomato.getProductionDate() == tomatoCopy.getProductionDate()",
				tomato.getProductionDate() == tomatoCopy.getProductionDate(), false);
		printTestData("copy constractor aliasing", "tomato.getExpiryDate() == tomatoCopy.getExpiryDate()",
				tomato.getExpiryDate() == tomatoCopy.getExpiryDate(), false);

		printTestData("defualt name", "cabbage.getName().equals(\"item\")", cabbage.getName().equals("item"), true);
		printTestData("defualt caltalogue", "cabbage.getCatalogueNumber() == 9999",
				cabbage.getCatalogueNumber() == 9999, true);
		printTestData("defualt caltalogue", "f1.getCatalogueNumber() == 9999", f1.getCatalogueNumber() == 9999, true);
		printTestData("defualt quantity", "cabbage.getQuantity() == 0", cabbage.getQuantity() == 0, true);
		printTestData("defualt production date", "cabbage.getProductionDate().equals(d4)",
				cabbage.getProductionDate().equals(d4), true);
		printTestData("defualt expiry date", "cabbage.getExpiryDate().equals(d4.tomorrow())",
				cabbage.getExpiryDate().equals(d4.tomorrow()), true);
		printTestData("defualt min temperature", "cabbage.getCatalogueNumber() == 9999",
				cabbage.getMinTemperature() == 20, true);
		printTestData("defualt max temperature", "cabbage.getMaxTemperature() == 30", cabbage.getMaxTemperature() == 30,
				true);
		printTestData("defualt price", "cabbage.getPrice() == 1", cabbage.getPrice() == 1, true);
		...
		printTestData("setExpiryDate", "cabage.getExpiryDate().equals(d_1)", tomato.getExpiryDate().equals(d_1), false);
		tomato.setExpiryDate(tomato.getProductionDate());
		printTestData("setExpiryDate", "cabage.getExpiryDate().equals(tomato.getProductionDate())",
		...
		printTestData("equals", "cabbage.equals(cabage)", cabbage.equals(cabbage), true);
		printTestData("equals", "cabbage.equals(carrot)", cabbage.equals(carrot), false);
		printTestData("equals", "cabbage.equals(new FoodItem(cabage))", cabbage.equals(new FoodItem(cabbage)), true);
		...
		if (isWorked)
		{
			...
			System.out
					.println("Check your desktop, you should see a new text file named with the name that you choosed");
			System.out.println("the defualt file name is \"FoodItem_Tester_Results_49Q14srBqj\"");
			System.out.println("If you did failed in somthing you can search the word \"Failed\" in the text file");
		}
		else
			System.out.println("Somthing went wrong when tried to export the test results");
	}
